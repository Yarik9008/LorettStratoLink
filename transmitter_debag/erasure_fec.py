...
Файл разбивается на K блоков данных. Кодер RS строит M блоков чётности,
сгруппированных так, чтобы каждый кодовый блок RS помещался в лимит GF(2^8) (255 символов).
Блоки данных распределены по группам (интерливинг), чтобы распределить пакетные потери.

Любая потеря до M_per_group блоков в группе восстанавливается декодером.

Формат пакета (256 байт):
  ...
  9         2       k_data         число блоков данных
  11        2       n_total        K + M (всего блоков)
  13        4       file_size      размер файла
  17        1       file_type      0x01=JPEG, 0x02=WebP, 0x00=raw
  18        1       m_per_group    чётность на группу RS
  19        1       num_groups      число групп RS
  20        200     payload        полезные данные блока
  220       4       crc32          CRC-32 байт [1..219]
  224       32      reserved
  Итого: 256 байт
"""
...
# Размеры пакета и блока
PKT_SIZE = 256
BLOCK_PAYLOAD = 200
HEADER_SIZE = 20
CRC_SIZE = 4
RESERVED_SIZE = PKT_SIZE - HEADER_SIZE - BLOCK_PAYLOAD - CRC_SIZE  # 32
RS_MAX = 255  # максимум символов в кодовом слове RS в GF(2^8)
...
# Алфавит base-40 для кодирования позывного (6 символов → 4 байта)
_BASE40 = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ-_. "


def encode_callsign(call: str) -> int:
    """Позывной до 6 символов → 32-битное число в кодировке base-40 (big-endian в пакете)."""
...
def decode_callsign(val: int) -> str:
    """Число base-40 → строка позывного (до 6 символов)."""
    chars = []
    for _ in range(6):
...
def detect_file_type(data: bytes) -> int:
    """Определение типа файла по магическим байтам: JPEG, WebP или raw."""
    if data[:2] == b"\xff\xd8":
        return FTYPE_JPEG
...
def _rs_group_params(k: int, fec_ratio: float) -> tuple[int, int, int]:
    """Вычисление (g_size, m_g, num_groups) для RS в пределах GF(2^8).

    Возвращает: блоков данных в группе, блоков чётности в группе, число групп.
    """
...
class FECPacket:
    ...
    def is_parity(self) -> bool:
        """True, если блок — чётность (block_id >= K)."""
        return self.block_id >= self.k_data

    # Формат заголовка: sync, type, callsign(4), image_id, block_id, k, n, file_size, file_type, m_per_group, num_groups
    _HDR = ">BBIBHHHIBBB"
    ...
    def to_bytes(self) -> bytes:
        ...
        body = hdr[1:] + pl  # CRC считается от байта type до конца payload (без sync)
    ...
    def from_bytes(cls, raw: bytes) -> Optional["FECPacket"]:
        """Разбор 256 байт в FECPacket; при неверном sync/type/CRC возвращается None."""
...
# ═══════════════════════════════════════════════════════════════
#  Кодер: файл → список FEC-пакетов (K data + M parity)
# ═══════════════════════════════════════════════════════════════
...
class ErasureEncoder:
    ...
    def encode_bytes(self, data: bytes) -> list[FECPacket]:
        ...
        # data_matrix[i] — список из BLOCK_PAYLOAD байт i-го блока
        data_matrix = [
            list(padded[i * BLOCK_PAYLOAD : (i + 1) * BLOCK_PAYLOAD])
            for i in range(k)
        ]

        # Чётность считается по группам RS; блоки данных распределены по группам (интерливинг)
...
# ═══════════════════════════════════════════════════════════════
#  Декодер: накопление блоков, восстановление файла при наличии ≥ K блоков
# ═══════════════════════════════════════════════════════════════
...
class ErasureDecoder:
    ...
    def add_packet(self, pkt: FECPacket) -> bool:
        """Добавить блок; при смене image_id — сброс состояния. Возвращает True."""
    ...
    def decode(self) -> Optional[bytes]:
        """Восстановить файл из накопленных блоков (RS-декодирование по группам). При ошибке — None."""
        ...
        try:
            for g in range(ng):
                ...
                # Позиции стираний: 0..g_size-1 — данные/дополнение, g_size..g_size+m_g-1 — чётность
                erase_pos = []
    ...
    def assemble_partial(self) -> bytes:
        """Собрать файл из уже полученных блоков (без RS); если decode() уже был — вернуть его результат."""
